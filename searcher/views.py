...
from .models import Title, Creator, Response
import json
import requests
...
def search(request):
    if request.method == 'POST':
        ...
        elif 'creator' in req["action"]["detailParams"]:
            req_creator = req["action"]["detailParams"]["creator"]["value"]
            Creator.objects.create(user_id = req_user_id, creator = req_creator)
        else:
            print("실패")
...
def getTitleUrl(target):
    return url + 'title' + others + target

def getCreatorUrl(target):
    return url + 'creator' + others + target

#def getPosition(callNumber):
#        response = requests.get('http://147.46.181.235/Mobile/BOOK/book_location_info.php?call_num=' + callNumber)
#        html = response.text
#        soup = BeautifulSoup(html, 'html.parser')
#        room = soup.select('#main_info > #cont_tit > .txtLayer')[0].text
#        bookshelf, _callNumber = soup.select('#main_info > #info_location_txt > .txtInfo')[0].text.split('청구기호 : ')
#        print(room, bookshelf, _callNumber)
#        answer = f"{room} {bookshelf}"
#
#        return answer
@csrf_exempt

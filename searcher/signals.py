...
def title_post_save(sender, **kwargs):
    req = kwargs['instance']
    response = searchTitle(req.title)
    html = response.text
...
def creator_post_save(sender, **kwargs):
    req = kwargs['instance']
    response = searchCreator(req.creator)
    html = response.text
    soup = BeautifulSoup(html, 'html.parser')
    titles = soup.select('.EXLResultTitle')
    #authors = soup.select('.EXLResultAuthor')
    #detail = soup.select('.EXLResultFourthLine')
    availabilities = soup.select('.EXLResultAvailability')

    Response.objects.create(
            user_id = req.user_id,
            title0 = titles[0].text,
            detail0 = availabilities[0].text,

            title1 = titles[1].text,
            detail1 = availabilities[1].text,

            title2 = titles[2].text,
            detail2 = availabilities[2].text,
    )
